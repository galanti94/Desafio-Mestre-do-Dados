...
from plotando_graficos import plotar
    ...
    if lista_formacao[i] not in formacao:
        for j in range(0, tamanho_lista_nomes):
            if lista_formacao[i] == lista_formacao[j]:
                contador_formacao += 1
    # Contando a frequencia de nomes, ano de nascimento e formacao
    if contador_nomes > 1:
...
# todo: Os gráficos estão péssimos, arrumar formatação
